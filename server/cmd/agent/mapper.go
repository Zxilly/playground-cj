...
func (m *Mapper) LineCol8Position(line, col8 int) (protocol.Position, error) {
	// Report a bug for inputs that are invalid for any file content.
	if line < 1 {
		return protocol.Position{}, fmt.Errorf("invalid 1-based line number: %d", line)
	}
	if col8 < 1 {
		return protocol.Position{}, fmt.Errorf("invalid 1-based column number: %d", col8)
	}

	m.initLines()
	line0 := line - 1 // 0-based
	if !(0 <= line0 && line0 < len(m.lineStart)) {
		return protocol.Position{}, fmt.Errorf("line number %d out of range (max %d)", line, len(m.lineStart))
	}
	...
	if end > len(m.Content) {
		return protocol.Position{}, fmt.Errorf("column is beyond end of file")
	} else if line0+1 < len(m.lineStart) && end >= m.lineStart[line0+1] {
		return protocol.Position{}, fmt.Errorf("column is beyond end of line")
	}

	char := UTF16Len(m.Content[start:end])
	return protocol.Position{Line: uint32(line0), Character: uint32(char)}, nil
}

// -- conversions from byte offsets --

// OffsetLocation converts a byte-offset interval to a protocol (UTF-16) location.
func (m *Mapper) OffsetLocation(start, end int) (protocol.Location, error) {
...
func (m *Mapper) OffsetRange(start, end int) (protocol.Range, error) {
	...
	if err != nil {
		return protocol.Range{}, fmt.Errorf("start: %v", err)
	}
	endPosition, err := m.OffsetPosition(end)
	if err != nil {
		return protocol.Range{}, fmt.Errorf("end: %v", err)
	}
	return protocol.Range{Start: startPosition, End: endPosition}, nil
}

// OffsetPosition converts a byte offset to a protocol (UTF-16) position.
func (m *Mapper) OffsetPosition(offset int) (protocol.Position, error) {
...
func (m *Mapper) RangeOffsets(r protocol.Range) (int, int, error) {
...
func (m *Mapper) PositionOffset(p protocol.Position) (int, error) {
	m.initLines()
...
func (m *Mapper) PosPosition(tf *token.File, pos token.Pos) (protocol.Position, error) {
...
func (m *Mapper) PosLocation(tf *token.File, start, end token.Pos) (protocol.Location, error) {
...
func (m *Mapper) PosRange(tf *token.File, start, end token.Pos) (protocol.Range, error) {
	startOffset, endOffset, err := safetoken.Offsets(tf, start, end)
	if err != nil {
...
func (m *Mapper) NodeRange(tf *token.File, node ast.Node) (protocol.Range, error) {
	return m.PosRange(tf, node.Pos(), node.End())
}

// RangeLocation pairs a protocol Range with its URI, in a Location.
func (m *Mapper) RangeLocation(rng protocol.Range) protocol.Location {
	return protocol.Location{URI: protocol.DocumentURI(m.URI), Range: rng}
}
...
func (mr MappedRange) Range() protocol.Range {
...
func (mr MappedRange) Location() protocol.Location {
